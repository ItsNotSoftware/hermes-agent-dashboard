...
func (p *controlPage) Build() fyne.CanvasObject {
	...
	loopsCard := widgets.Card("TODOS / OPEN LOOPS", container.NewVBox(
		widgets.Label("TASKS", widgets.ColMuted, 9, true),
		container.NewScroll(p.todos),
		widgets.Label("LOOPS", widgets.ColMuted, 9, true),
		container.NewScroll(p.loops),
		widgets.Label("INBOX", widgets.ColMuted, 9, true),
...
func (p *controlPage) Update(snap store.Snapshot) {
	...
	for _, ev := range snap.Calendar.Events {
		line1 := widgets.Label(ev.Day+"  "+ev.Clock, widgets.ColCyan, 10, true)
		line2 := widgets.Label(ev.Title, widgets.ColText, 11, false)
	...
	if len(snap.Calendar.Events) == 0 {
		...
		p.calendar.Add(widgets.Label(msg, widgets.ColFaint, 11, false))
	}
	p.calendar.Refresh()
...
func fillItems(c *fyne.Container, items []loops.Item, empty string) {
	...
	for _, it := range items {
		c.Add(container.NewHBox(
			widgets.Label("• ", widgets.ColMuted, 11, false),
			widgets.Label(it.Title, widgets.ColText, 11, false),
			widgets.Label(it.Source, widgets.ColFaint, 9, false),
		))
	}
	if len(items) == 0 {
		c.Add(widgets.Label(empty, widgets.ColFaint, 10, false))
	}
	c.Refresh()
}
...
func commandRow(c derive.Command) fyne.CanvasObject {
	...
	title := widgets.Label(c.Title, widgets.ColText, 11, true)
	detail := widgets.Label(c.Detail, widgets.ColMuted, 10, false)
